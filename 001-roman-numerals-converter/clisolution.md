...
aws ec2 create-security-group \
    --group-name okde09_roman_numbers_converter_sec_grp \
    --description "This Sec Group is to allow ssh and http from anywhere"
```

- We can check the security Group with these commands
```bash
aws ec2 describe-security-groups --group-names okde09_roman_numbers_converter_sec_grp
```
...
```
```bash
aws ec2 run-instances --image-id $LATEST_AMI --count 1 --instance-type t2.micro --key-name okt-aws --security-groups okde09_roman_numbers_converter_sec_grp --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=okt_roman_numbers}]' --user-data file:///home/ec2-user/userdata.sh
or
